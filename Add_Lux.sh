...
#->entry       (entry_date, real_date, _dev_id*, [seq])
if sqlite3 $DATABASE "PRAGMA foreign_keys = ON; PRAGMA journal_mode = WAL; begin; INSERT INTO entry VALUES('$EN_DATE', '$RL_DATE', '$DEV_ID', (select max(seq) from entry) + 1); INSERT INTO lux VALUES($LUX,(select max(seq) from entry)); commit;" ; then
    echo "s1"
    exit
